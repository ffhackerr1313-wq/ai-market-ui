...
const MAX_WIDTH_CLASS: Record<MaxWidthToken, string> = {
  ...
  xl: "max-w-xl", "2xl": "max-w-2xl", "3xl": "max-w-3xl", none: "",
};
...
type Side = "L" | "R";
type TileConfig = {
  aspectRatio: string; perspective: number;
  maxTilt: number; maxBlur: number; rounded: string;
};

function Tile({
  src, side, config, index, renderOverlay,
}: {
  src: string;
  side: Side;
  config: TileConfig;
  ...
  const canShowOverlay = renderOverlay !== undefined && index !== undefined;
    ...
    return (
      <figure ref={ref} className="relative z-10 m-0">
        <div
          ...
          style={{ aspectRatio, borderRadius: rounded, cursor: canShowOverlay ? "pointer" : "default" }}
          onClick={() => canShowOverlay && setOverlayOpen(o => !o)}
        >
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: `url("${src}")` }} />
          {canShowOverlay && overlayOpen && (
            <div style={{ position: "absolute", inset: 0 }}>{renderOverlay!(index!)}</div>
          )}
        </div>
      </figure>
    );
    ...
    <motion.figure ref={ref} className="relative z-10 m-0" style={{ perspective, willChange: "transform" }}>
      <motion.div
        ...
        style={{
          ...
          cursor: canShowOverlay ? "pointer" : "default",
        }}
        onClick={() => canShowOverlay && setOverlayOpen(o => !o)}
      >
        <motion.div
        ...
        {/* Quote overlay — appears on click, clipped to image bounds */}
        <AnimatePresence>
          {canShowOverlay && overlayOpen && (
            <motion.div
              key="overlay"
...
export type ScrollTiltedGridProps = {
  ...
  rounded?: string;
  className?: string;
  /** Optional per-tile overlay renderer. Receives the tile index (wraps for loop mode). */
  renderOverlay?: (index: number) => React.ReactNode;
};
      ...
      <div className={gridClass}>
        {items.map((src, i) => (
          <Tile
            ...
            side={i % 2 === 0 ? "L" : "R"}
            config={config}
            index={renderOverlay ? i % images.length : undefined}
